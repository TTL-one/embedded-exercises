...
static int g_recvFlag = 0;
int read_fd_raw_data(int fd, char * data){
	char temp = 0;
	int iRet = 0;
	int flag = 0;
	while(1){
...
int parse_fd_raw_data(char *buf, char *time, char *lat, char *ns, char *lng, char *ew)
{
	char tmp[10];
	if(buf[0]!='$'){
		return -1;
	}
	else if(strcmp(buf+3,"GGA")!=0){
		return -1;
